...
from nets.nets import GenericNC, BasicCNN, BasicMLP
...
def get_dataloader(args):
    ...
    # Create random train and validation splits
...
def main():
    ...
    # Training settings
    parser = argparse.ArgumentParser(description='DDN-NC')
    ...
    # model = Net().to(device) # TODO: fix
    model = GenericNC(pre_net, args.size[0], args.net, args.mat_type, args.method)
    model = model.to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr)
    
    # should probably just use AdamW but this might be fun later..
